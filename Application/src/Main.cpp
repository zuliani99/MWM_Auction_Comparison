...
int main(int argc, const char* argv[])
{
	...
	stream << "Edge per part,Execution Time MWM,Seconds MWM,Total Cost MWM,"
		...
		"Execution Time cAU,Seconds cAU,Total Cost cAU,Iterations cAU,"
	...
	for (int n = min; n <= max; ++n)
	{
		...
		std::vector<Duration> elapsed_au(3); //elapsed_mwm, elapsed_au;
		std::vector<Weight> total_cost_au(3); // total_cost_mwm, total_cost_au;
		Weight best_total_cost_au = 0;
		Duration best_elapsed_au;
		int best_n_iter_au;
		std::string au_winner;
		std::vector<int> n_iteration_au(3);
		...
		stream << n << "," << fmt{ elapsed_mwm } << "," << (elapsed_mwm / 1.0s) << "," << (total_cost_mwm) << "," << // / 10'000.0
			...
			fmt{ elapsed_au[2] } << "," << (elapsed_au[2] / 1.0s) << "," << (total_cost_au[2]) << "," << n_iteration_au[2] << "," <<
